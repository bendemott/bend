...
class PbApplication(EasyReferenceable):
	'''
	...
	def new(self, modulename, classname):
		...
		if(not cls):
			raise NameError("Module [%s] is missing the attribute/class [%s] in %s" % (modname, classname, os.path.abspath(fpath)) )
		#Ensure the class will work with Twisted / Inherits from referenceable!
		if( not issubclass(cls, pb.Referenceable) ):
			raise TypeError("Class cannot be loaded - [%s.%s] must be a subclass of 'twisted.spread.pb.Referenceable' " % (modname, cls.__name__) )
