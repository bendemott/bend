...
def main(argv):
	...
	print "------------"
	if verbose:
		tokens = NysiisFilter(sinks.pop())
		term = tokens.addAttribute(lucene.TermAttribute.class_)
		while tokens.incrementToken():
			print "[%s]" % term.term(),
		print "\n  REPLACE"
	tokens = NysiisFilter(sinks.pop(), replace=True)
	term = tokens.addAttribute(lucene.TermAttribute.class_)
	while tokens.incrementToken():
		print "[%s]" % term.term(),
	print
...
class NysiisFilter(lucene.PythonTokenFilter):
	"""
	The ``NysiisFilter`` class is a PyLucene Filter that converts tokens
	to their phonetic representation using the New York State
	Identification and Intelligence System algorithm.
	...
	*term* (``lucene.TermAttribute``) is used for accessing the value of a
	...
	def __init__(self, in_stream, truncate=None, replace=None):
		"""
		Initializes the ``NysiisFilter`` instance.
