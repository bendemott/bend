# coding: utf-8
"""
This module contains a PyLucene filter that removes non-word characters.
"""
...
def main(argv):
	...
	print "WordCharFilter"
	print "--------------"
	if verbose:
		tokens = WordCharFilter(sinks.pop())
		term = tokens.addAttribute(lucene.TermAttribute.class_)
		while tokens.incrementToken():
			print "[%s]" % term.term(),
		print "\n  REPLACE"
	tokens = WordCharFilter(sinks.pop(), replace=True)
	term = tokens.addAttribute(lucene.TermAttribute.class_)
	while tokens.incrementToken():
		print "[%s]" % term.term(),
	print
...
class WordCharFilter(lucene.PythonTokenFilter):
	"""
	The ``WordCharFilter`` class is a PyLucene Token Filter that removes
	non-word characters.
	
	Class Attribute:
		
	*word_chars* (``re.RegexObject``) matches sequences of word
	characters.
	...
	*term* (``lucene.TermAttribute``) is used for accessing the value of a
	...
	def __init__(self, in_stream, replace=None):
		"""
		Initializes the ``WordCharFilter`` instance.
	...
	def filter(self, tokens, replace):
		...
		*replace* (``bool``) is whether tokens created by this filter should
		...
		for token in tokens:
			...
			for match in word_chars.finditer(val):
				slice = match.group(0)
