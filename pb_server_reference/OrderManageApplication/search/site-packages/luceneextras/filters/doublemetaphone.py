...
def main(argv):
	...
	print "DoubleMetaphoneFilter"
	print "---------------------"
	if verbose:
		tokens = DoubleMetaphoneFilter(sinks.pop())
		term = tokens.addAttribute(lucene.TermAttribute.class_)
		while tokens.incrementToken():
			print "[%s]" % term.term(),
		print "\n  REPLACE"
	tokens = DoubleMetaphoneFilter(sinks.pop(), replace=True)
	term = tokens.addAttribute(lucene.TermAttribute.class_)
	while tokens.incrementToken():
		print "[%s]" % term.term(),
	print
...
class DoubleMetaphoneFilter(lucene.PythonTokenFilter):
	"""
	The ``DoubleMetaphoneFilter`` class is a PyLucene Filter that converts
	tokens to their phonetic representation using the Double Metaphone
	algorithm.
	
	Instance Attribute:
		
	*dmeta* (``fuzzy.DMetaphone``) is the double metaphone instance.
	...
	*truncate* (``int``) is the maximum length of a double metaphone
	string.
	"""
	
	def __init__(self, in_stream, truncate=None, replace=None):
		"""
		Initializes the ``DoubleMetaphoneFilter`` instance.
		
		*in_stream* (``lucene.PythonTokenStream``) is the token input
		stream.
		
		*truncate* (``int``) is the maximum length of a NYSIIS string.
		Default is ``None``.
		...
		*replace* (``bool``) is whether tokens created by this filter should
		...
		if truncate:
			if not isinstance(truncate, int):
				raise TypeError("truncate:%r is not an ``int``." % length)
			elif length < 1:
				raise ValueError("truncate:%r must be greater than 0." % length)
		...
		self.truncate = truncate or None
		...
		*replace* (``bool``) is whether tokens created by this filter should
		...
		for token in tokens:
			if not replace:
				...
				final.append(token)
			# Run token through double metaphone.
			pos = token[1] if replace else 0
			pri, sec = dmeta(token[0])
			if pri:
				pri = pri[:truncate]
				if pri not in found:
					found.add(pri)
					final.append((pri, pos, token[2], token[3]))
					pos = 0
			if sec:
				sec = sec[:truncate]
				if sec not in found:
					found.add(sec)
					final.append((sec, pos, token[2], token[3]))
		return final
