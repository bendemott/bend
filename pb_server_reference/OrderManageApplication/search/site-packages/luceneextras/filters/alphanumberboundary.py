# coding: utf-8
"""
This module contains a PyLucene filter that separates numbers from
words.
"""
...
__all__ = ['AlphaNumberBoundaryFilter']
...
def main(argv):
	...
	print "AlphaNumberBoundaryFilter"
	print "-------------------------"
	if verbose:
		tokens = AlphaNumberBoundaryFilter(sinks.pop())
		term = tokens.addAttribute(lucene.TermAttribute.class_)
		while tokens.incrementToken():
			print "[%s]" % term.term(),
		print "\n  REPLACE"
	tokens = AlphaNumberBoundaryFilter(sinks.pop(), replace=True)
	term = tokens.addAttribute(lucene.TermAttribute.class_)
	while tokens.incrementToken():
		print "[%s]" % term.term(),
	print
...
class AlphaNumberBoundaryFilter(lucene.PythonTokenFilter):
	"""
	The ``AlphaNumberBoundaryFilter`` class is a PyLucene Token Filter
	that separates numbers from words.
	
	Class Attributes:
		
	*seq* (``re.RegexObject``) matches sequences of either digits or
	...
	*term* (``lucene.TermAttribute``) is used for accessing the value of a
	...
	def __init__(self, in_stream, replace=None):
		"""
		Initializes the ``AlphaNumberBoundaryFilter`` instance.
		...
		*replace* (``bool``) is whether tokens created by this filter should
		...
		for token in tokens:
			if not replace:
				...
				final.append(token)
			# Split on sequences of digits or non-digits.
			val = token[0]
			pos = token[1] if replace else 0
			off_start = token[2]
			off_end = token[3]
			is_source = off_end - off_start == len(val)
			for match in seq.finditer(val):
				slice = match.group(0).strip()
